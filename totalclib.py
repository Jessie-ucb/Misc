...
class TotalCLib(object):
	...
	def smpl_fit_single(self,target_joints,betas_init,pose_init,trans_init,reg_type,gender='male',fit_shape=True,show_iter=True):
		...
		pose_c[:] = pose_init.flatten().tolist()
		betas_c[:] = betas_init.flatten().tolist()
		trans_c[:] = trans_init.flatten().tolist()
		...
		#must use frombuffer and copy here. otherwise there maybe some unexpected results
	...
	def load_SMPLModel(self,modelpath,gender):
		...
		with open(modelpath,'rb') as f:
			SMPLpkl = pickle.load(f,encoding='latin1')
